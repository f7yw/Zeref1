...
export function fmt(n, user = null) {
  if (user?.infiniteResources) return '∞ 🪙'
  return Number(n || 0).toLocaleString('en') + ' 🪙'
}

/**
 * Ensure a VIP user's resources are truly infinite in DB.
 */
export function syncVipResources(user, jid = null) {
  ...
  user.energy            = MAX_ENERGY
  user.lastEnergyRegen   = Date.now()
  if ((user.money  || 0) < 1_000_000) user.money  = 2_000_000_000
  if ((user.bank   || 0) < 1_000_000) user.bank   = 2_000_000_000
