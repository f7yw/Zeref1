...
/**
 * Returns true if the JID is a registered owner/developer.
 */
...
/**
 * Returns the tier of a user: 'dev' | 'vip' | 'normal'
 */
export function getTier(jid) {
  if (isOwner(jid)) return 'dev'
  if (isVip(jid))   return 'vip'
  return 'normal'
}

/**
 * Returns true if user is registered (has a confirmed account).
 */
export function isRegistered(jid) {
  ...
  const resolvedJid = (jid?.endsWith?.('@lid') && global.lidPhoneMap?.[jid])
    ? global.lidPhoneMap[jid]
    : jid
  const u = global.db?.data?.users?.[resolvedJid] || global.db?.data?.users?.[jid]
  return !!(u?.registered)
}

/**
 * Tier badge for command headers.
 * Format: "👑 مطور · ✓ مسجّل" / "💎 مميز · ✗ غير مسجّل" / "👤 عادي · ✓ مسجّل"
 */
export function tierBadge(jid, opts = {}) {
  const t   = getTier(jid)
  const reg = isRegistered(jid)
  const tierTxt =
    t === 'dev' ? '👑 مطور' :
    t === 'vip' ? '💎 مميز' :
                  '👤 عادي'
  if (opts?.short) return tierTxt
  const regTxt = reg ? '✓ مسجّل' : '✗ غير مسجّل'
  return `${tierTxt} · ${regTxt}`
}
