...
export async function createSubBot(phone, ownerJid = null) {
  ...
  subConn.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'close') {
      ...
      // إذا تم حذف البوت يدوياً → لا إعادة اتصال ولا إشعار "انفصل"
      if (subConn.__destroyed) {
        console.log(`[JADIBOT] 🗑️ Sub-bot ${num} closed cleanly after manual destroy.`)
        if (subConn.__pendingReconnect) { try { clearTimeout(subConn.__pendingReconnect) } catch {} }
      ...
      if (wasFullyPaired && (RECONNECT_CODES.has(code) || code === undefined)) {
        ...
        subConn.__pendingReconnect = setTimeout(async () => {
          subConn.__pendingReconnect = null
          if (subConn.__destroyed) return // أُلغيت بعد جدولة المحاولة
          try {
...
export async function destroySubBot(phone) {
  ...
  if (sub) {
    ...
    if (sub.__pendingReconnect) { try { clearTimeout(sub.__pendingReconnect) } catch {} ; sub.__pendingReconnect = null }
  }
